...
public class Janela extends JFrame implements Observador {
	...
	@Override
	public void navegarProdutoAnterior() {
		cardLayout.previous(jpProdutos);
	}

	@Override
	public void navegarProdutoPosterior() {
	...
	class ItensTableModel extends AbstractTableModel {
		...
		public int getRowCount() {
			return pedido.getTabelaPedidos().size() + 1;
		}

		@Override
		public Object getValueAt(int rowIndex, int colIndex) {
			if (rowIndex == pedido.sizePedido()) {
				if (colIndex == 2) {
					return pedido.getPrecoTotal();
					/*double total = 0;
					for (Pedido ped : pedido.getTabelaPedidos()) {
				...
				Pedido p = pedido.getTabelaPedidos().get(rowIndex);
				switch (colIndex) {
				case 0:
	...
	public Janela() {
		...
		this.pedido.addObservador(this);
		...
		pedido.getTabelaProdutos().add(new Produto("cal\u00E7a", 83));
		pedido.getTabelaProdutos().add(new Produto("camisa", 57));
		pedido.getTabelaProdutos().add(new Produto("gravata", 15.7));
		pedido.getTabelaProdutos().add(new Produto("tenis", 235.8));
	...
	private void initComponents() {
		...
		for (Produto p : pedido.getTabelaProdutos()) {
